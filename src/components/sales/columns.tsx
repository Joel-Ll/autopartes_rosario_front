...
export const columns: ColumnDef<SaleComun>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (!client?.name)
        return <div className="font-medium text-sm">S/N</div>;
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-col">
        <span className="text-xs text-muted-foreground">Productos: {row.original.itemsCount}</span>
        <span className="text-xs text-muted-foreground">Servicios: {row.original.servicesCount}</span>
      </div>
    )
  },

  {
    ...
    header: 'Método Pago',
    cell: ({ row }) => {
      const method = row.original.transactions.length > 1 ? 'EFECTIVO y QR' : row.original.transactions[0].method === 'cash' ? 'EFECTIVO' : 'QR';
      return (
        ...
        <div className='text-right'>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => navigate(`/sales/detail/${saleId}`)} >
                <Eye />
                Ver
              </DropdownMenuItem>
              {/* <DropdownMenuItem >
