...
export default function DetailProduct({ product, openView, setOpenView }: Props) {

  const getCategoryColor = (categoria: string) => {
    const colors: Record<string, string> = {
      electronica: "bg-primary/10 text-primary border-primary/20",
      alimentos: "bg-accent/10 text-accent border-accent/20",
      textil: "bg-secondary/10 text-secondary-foreground border-secondary/20",
      hogar: "bg-muted text-muted-foreground border-muted-foreground/20",
      otros: "bg-card text-card-foreground border-border",
    };
          ...
          <DialogTitle>
            <div className="relative h-96 overflow-hidden group">
              ...
              <div className="absolute top-4 left-4">
                <Badge
                  variant="outline"
                  className={`${getCategoryColor(product.category.name)} backdrop-blur-sm`}
                >
                ...
                <div className="flex items-end justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-white/90 drop-shadow-md">
                      Código: <span className="font-mono font-semibold">{product.code}</span>
                    </p>
                  </div>
          ...
          <div className="p-6 space-y-4">
            <div className="flex justify-between">
              ...
              <div className="space-y-1">
                ...
                <p className="text-base font-semibold text-foreground">{product.currentStock} {product.unidadMedidaAbr}</p>
              </div>
            </div>
          </div>
        </DialogContent>
